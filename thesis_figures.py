...
# ═════════════════════════════════════════════════════════════════════════════
print('[2/12] Zero-shot vs few-shot')
...
# ═════════════════════════════════════════════════════════════════════════════
print('[3/12] Subject × condition heatmap')
...
# ═════════════════════════════════════════════════════════════════════════════
print('[4/12] Failure mode fix rate')
if fm is None:
...
# ═════════════════════════════════════════════════════════════════════════════
# FIG 6 — Confidence calibration by failure mode
# ═════════════════════════════════════════════════════════════════════════════
print('[6/12] Confidence calibration by FM')
if fm is None:
    print('  Skipped — no failure modes CSV.')
else:
    bl = zs[zs['condition_id'] == 0].merge(fm[['idx', 'failure_mode']], on='idx', how='left')
    correct    = bl[bl['is_correct']]['confidence'].dropna()
    fm1_err    = bl[~bl['is_correct'] & (bl['failure_mode'] == 'FM1')]['confidence'].dropna()
    fm2_err    = bl[~bl['is_correct'] & (bl['failure_mode'] == 'FM2')]['confidence'].dropna()
    u, p_mw = stats.mannwhitneyu(fm1_err, fm2_err, alternative='two-sided')

    fig, axes = plt.subplots(1, 2, figsize=(12, 4.2))

    ax = axes[0]
    for data, lbl, color in [
        (correct, f'Correct (μ={correct.mean():.2f}, n={len(correct)})', PALETTE[2]),
        (fm1_err, f'FM1 – Param. Override (μ={fm1_err.mean():.2f}, n={len(fm1_err)})', PALETTE[5]),
        (fm2_err, f'FM2 – Reasoning Fail (μ={fm2_err.mean():.2f}, n={len(fm2_err)})', PALETTE[0]),
    ]:
        if len(data) > 1:
            data.plot.kde(ax=ax, label=lbl, color=color, linewidth=1.6)
    ax.set_xlabel('Model Confidence')
    ax.set_ylabel('Density')
    ax.set_title('(a) Confidence Distribution by Outcome')
    ax.set_xlim(0, 1)
    ax.legend()
    sns.despine(ax=ax)

    ax = axes[1]
    bp = ax.boxplot([correct, fm1_err, fm2_err],
                    labels=['Correct', 'FM1 Errors', 'FM2 Errors'],
                    patch_artist=True,
                    medianprops={'color': 'black', 'linewidth': 1.8},
                    flierprops={'marker': 'o', 'markersize': 3, 'alpha': 0.4})
    for patch, color in zip(bp['boxes'], [PALETTE[2], PALETTE[5], PALETTE[0]]):
        patch.set_facecolor(color)
        patch.set_alpha(0.7)
    ax.set_ylabel('Model Confidence')
    ax.set_title('(b) Confidence by Outcome / FM Type')
    ax.text(0.98, 0.97,
            f'FM1 vs FM2 Mann-Whitney\nU = {u:.0f}, p = {p_mw:.3f}',
            transform=ax.transAxes, ha='right', va='top',
            fontsize=8, color='#555555')
    sns.despine(ax=ax)

    fig.suptitle('Confidence Calibration per Failure Mode (Zero-Shot Baseline)', y=1.01)
    plt.tight_layout()
    savefig('fig6_confidence_calibration_by_fm.png')


# ═════════════════════════════════════════════════════════════════════════════
# FIG 7 — Cross-mode hardcore failures
# ═════════════════════════════════════════════════════════════════════════════
print('[7/12] Cross-mode hardcore failures')
def get_hardcore(df):
    pivot = df.pivot_table(index='idx', columns='condition_id',
                           values='is_correct', aggfunc='first')
...
ax = axes[2]
all_wrong_conf = zs[(~zs['is_correct']) & (zs['condition_id'] == 0)]['confidence'].dropna()
hc_conf = both_meta['confidence'].dropna()
ax.hist(all_wrong_conf, bins=18, alpha=0.5, color=PALETTE[0], density=True,
        label=f'All ZS wrong (n={len(all_wrong_conf)})')
ax.hist(hc_conf, bins=10, alpha=0.75, color=PALETTE[5], density=True,
        label=f'Both-mode HC (n={len(hc_conf)})')
...
# ═════════════════════════════════════════════════════════════════════════════
print('[8/12] Cost-accuracy Pareto')
...
# ═════════════════════════════════════════════════════════════════════════════
# FIG 10 — ECE per condition
# ═════════════════════════════════════════════════════════════════════════════
print('[10/12] ECE per condition')
zs_sum_path = 'results/zero_shot/summary.csv'
fs_sum_path = 'results/few_shot/summary.csv'

if os.path.exists(zs_sum_path) and os.path.exists(fs_sum_path):
    zs_sum = pd.read_csv(zs_sum_path)
    fs_sum = pd.read_csv(fs_sum_path)

    fig, ax = plt.subplots(figsize=(9, 4.2))
    x = np.arange(len(COND_NAMES))
    w = 0.38
    ax.bar(x - w/2, zs_sum['ece'], w, label='Zero-Shot', color=PALETTE[0], alpha=0.85, zorder=3)
    ax.bar(x + w/2, fs_sum['ece'], w, label='Few-Shot',  color=PALETTE[1], alpha=0.85, zorder=3)
    ax.axhline(0.453, color='#555555', linestyle='--', linewidth=1.2,
               label='Dahl et al. (2024) baseline (0.453)', zorder=2)
    ax.set_xticks(x)
    ax.set_xticklabels([COND_NAMES[i] for i in range(8)], rotation=38, ha='right')
    ax.set_ylabel('ECE (lower = better calibrated)')
    ax.set_title('Expected Calibration Error per Condition')
    ax.legend()
    sns.despine(ax=ax)

    # Annotate FS anomaly
    for cid in [5, 7]:
        ece_val = float(fs_sum.loc[fs_sum['condition_id'] == cid, 'ece'].values[0])
        ax.annotate('*logprob\nanomaly',
                    xy=(cid + w/2, ece_val),
    ...
    plt.tight_layout()
    savefig('fig10_ece_per_condition.png')
else:
    print('  Skipped — summary.csv not found.')


# ═════════════════════════════════════════════════════════════════════════════
# FIG 11 — Condition 6 self-consistency agreement
# ═════════════════════════════════════════════════════════════════════════════
print('[11/12] Self-consistency agreement')
fig, axes = plt.subplots(1, 2, figsize=(10, 4.2))

for ax, df, label, color in zip(axes, [zs, fs], ['(a) Zero-Shot', '(b) Few-Shot'],
                                  [PALETTE[0], PALETTE[1]]):
    c6 = df[df['condition_id'] == 6].copy()
    if c6.empty:
        ax.text(0.5, 0.5, 'No data', ha='center', transform=ax.transAxes)
        continue
    c6['votes']    = (c6['confidence'] * 3).round().astype(int).clip(1, 3)
    c6['unanimous'] = c6['votes'] == 3
    grp = c6.groupby('unanimous').agg(
        n=('is_correct', 'count'),
        accuracy=('is_correct', 'mean')
    ).reset_index()
    grp['label'] = grp['unanimous'].map({True: '3/3 Unanimous', False: '2/1 Split'})

    x = np.arange(len(grp))
    bars = ax.bar(x, grp['accuracy'], color=[PALETTE[2], PALETTE[5]][:len(grp)],
...
fig.suptitle('Condition 6: Accuracy by Self-Consistency Agreement Level', y=1.01)
plt.tight_layout()
savefig('fig11_self_consistency_agreement.png')


# ═════════════════════════════════════════════════════════════════════════════
# FIG 12 — Overconfidence: correct vs wrong by condition
# ═════════════════════════════════════════════════════════════════════════════
print('[12/12] Overconfidence by condition')
fig, axes = plt.subplots(1, 2, figsize=(13, 4.5))

for ax, df, label in zip(axes, [zs, fs], ['(a) Zero-Shot', '(b) Few-Shot']):
    cids = list(range(8))
    conf_c = [df[(df['condition_id'] == c) &  df['is_correct']]['confidence'].mean() for c in cids]
    conf_w = [df[(df['condition_id'] == c) & ~df['is_correct']]['confidence'].mean() for c in cids]
    x = np.arange(8)
    w = 0.38
    ax.bar(x - w/2, conf_c, w, color=PALETTE[2], alpha=0.85, label='Correct', zorder=3)
    ax.bar(x + w/2, conf_w, w, color=PALETTE[5], alpha=0.85, label='Wrong',   zorder=3)
    for xi, (cc, cw) in enumerate(zip(conf_c, conf_w)):
        if not (np.isnan(cc) or np.isnan(cw)):
            ax.text(xi, max(cc, cw) + 0.025, f'{cc-cw:+.2f}',
...
fig.suptitle('Mean Confidence: Correct vs. Wrong Answers per Condition\n'
             '(labels show correct − wrong gap)', y=1.02)
plt.tight_layout()
savefig('fig12_overconfidence_by_condition.png')
